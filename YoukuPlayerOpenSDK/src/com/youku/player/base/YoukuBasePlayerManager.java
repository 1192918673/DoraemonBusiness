...
public abstract class YoukuBasePlayerManager extends IBasePlayerManager
    ...
    public boolean autoPlay = false;// 是否自动播放
    ...
    private void initPlayAndSurface() {
        ...
        // //MediaPlayerDelegate部分初始化
        mediaPlayerDelegate.mediaPlayer = getMediaPlayer(); // MediaPlayerDelegate部分初始化
        surfaceHolder = surfaceView.getHolder();
        surfaceHolder.addCallback(mediaPlayerDelegate.mediaPlayer); // 此处同时把surface
        // holder句柄赋值给了mediaplayer
        if (!isHighEnd)
        ...
        mediaPlayerDelegate.mediaPlayer
                .setOnErrorListener(new OnErrorListener() {
                    ...
                    public boolean onError(MediaPlayer mp, int what, int extra) {
                        Logger.d("PlayFlow",
                                "播放器出现错误 MediaPlayer onError what=" + what
                                        + " !!!");
                        if (mYoukuPlayerView != null)
                        ...
                        onLoadingFailError();
                        if (pluginManager == null) {
                            Logger.d("PlayFlow", "onError出现错误:" + what
                        ...
                        if (null != pluginManager) {
                            getBaseActivity().runOnUiThread(new Runnable() {
                                public void run() {
                                    ...
                                    pluginManager.onLoaded();
                                    //Fix 底层没有重写setonpreparedlistener，所以无法收到这个事件，但是我要是注册又怕引发新的异常。
                                    //所以暂时跟这个事件合并
                                    if (mPlayerListener != null) {
...
//					"ifautoplay", true);
    ...
    public void updatePlugin(final int pluginID) {
        Logger.e(TAG, "数组访问 updatePlugin:" + pluginID);
