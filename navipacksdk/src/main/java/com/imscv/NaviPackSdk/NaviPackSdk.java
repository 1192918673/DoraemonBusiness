...
public class NaviPackSdk {
     ...
     *
     * @param handlerId  NaviPack对象ID
     * @param position_x 路径点X坐标缓冲区
     * @param position_y 路径点Y坐标缓冲区
     * @param num        路径点数量
     * @return 返回值小于0，表示失败，等于0 表示成功
     * @since SetTargets函数，只有在NaviPack完成定位，并载入地图后，才会有效。该函数的参数位置，是指世界坐标系下的位置信息。
     */
    public int setTargets(int handlerId, int[] position_x, int[] position_y, int num) {
        return native_setTargets(handlerId, position_x, position_y, num);
    }
    ...
    private native int native_setTargets(int id, int position_x[], int position_y[], int num);
